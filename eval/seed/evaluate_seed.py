...
import torch
from internvl.model.attention_layer_utils import attention_layer_range_to_tag, parse_attention_layer_range
...
def evaluate_chat_model():
    ...
    for ds_name in args.datasets:
        ...
        for _, (pixel_values, questions, answers, indexes) in enumerate(tqdm(dataloader)):
            ...
            generation_config["attention_layer_range"] = args.small_model_attention_layer_range
            ...
            for question, pred, answer, index, large_answer, small_answer, original_confidence,  consistency_score, small_model_time, large_model_time  in zip(questions, preds, answers, indexes, large_answers, small_answers, original_confidences, consistency_scores, small_model_times, large_model_times):
                outputs.append({
                    'question_id': index,
                    'question': question,
                    'prediction': pred,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--small_model_attention_layer_range', type=str, default='all')
    ...
    args.small_model_attention_layer_range = parse_attention_layer_range(
        args.small_model_attention_layer_range, small_config.llm_config.num_hidden_layers
    )
    args.small_model_attention_layer_range_tag = attention_layer_range_to_tag(
        args.small_model_attention_layer_range, small_config.llm_config.num_hidden_layers
    )
    args.out_dir = os.path.join(
        args.out_dir,
        f"PruneLayer_{args.large_model_prune_layer}_PruneRatio_{args.large_model_prune_ratio}_{args.small_model_attention_layer_range_tag}",
    )
    os.makedirs(args.out_dir, exist_ok=True)
    print('small model attention layers:', args.small_model_attention_layer_range_tag)
    small_model_size = args.small_checkpoint.split("-")[-1]
    if small_model_size in ['1B','40B', '76B']:
